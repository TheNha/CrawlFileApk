...
class Crawler:
    ...
    def __get_new_app_list(self, popular_url):
        ...
        element = self.chrome.find_element_by_id('show-more-button')
        element.click()
        time.sleep(1)
    ...
    def __download_apk(self,package_name,download_url):
        ...
        try:
            ...
            data = r.content
            pos_begin = data.find(b'src="https://download.apkpure.com/b/apk/')
            pos_end =  data.find(b'"></iframe>', pos_begin)
            link = data[pos_begin:pos_end].decode('utf-8')
            link = link.replace("src=\"", "")
            #print(link)
            r = requests.get(link)
            with open(self.apk_directory + file_name,'wb') as apk:
                apk.write(r.content)
            # print("Download complete file : " + str(package_name))
    ...
    def crawl_new(self):
        ...
        global cate_size
        global cate_current
        cate_size = len(self.category_list)
        for category in self.category_list:
            cate_current = cate_current + 1
            ...
            if cate_current > 20:        
                ...
                self.download_fileapk(new_package_list)


            # updated_app_list = self.__get_app_detail(new_package_list)

            # self.db_connector.dow(updated_app_list, category_name)
    ...
    # nhant 
    def download_fileapk(self, package_list):
        global cate_size
        global cate_current
        size_pack = len(package_list)
        pack_current = 0
        for package_name in package_list:
            try:
                pack_current = pack_current + 1
                # apkpure.com에 패키지 이름으로 검색
                print("prcessing: " + str(pack_current) + "/" + str(size_pack) + " of " + str(cate_current) + "/" + str(cate_size))
                search_url = 'http://apkpure.com/search?q=' + package_name
                self.chrome.get(search_url)
                self.chrome.implicitly_wait(2)
                ...
                a_list = self.chrome.find_elements_by_class_name('da')
                ...
                try:
                    for a in a_list:
                        ...
                        # href링크에 패키지 이름있는것이 있으면 발견!
                        if package_name in link:
                            self.__download_apk(package_name, link)
                            """
                            self.chrome.get(link)
                            self.chrome.implicitly_wait(10)
                            """
                ...
                print("Loi khong download duoc file : " + str(package_name))
                continue
